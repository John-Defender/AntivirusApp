...
bool utilityRemove(std::wstring file_location)
{
	...
	if (DeleteFile(file_location.c_str())==0)
	{
		//_tremove(file_location.c_str());
		//boost::filesystem::remove(file_location);
		return true;
	}
	else
	{
		std::cout << "File can't be removed"<<strerror(errno);
	}
	return false;
}
...
/*
Bug fix: Opens the file and calculates the md5 and does not close the file and the bug has 
been fixed [FIXED]
*/
